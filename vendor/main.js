...
(function(){
	...
	var // jQuery and jQueryUI version
		...
		start = function(elFinder, editors, config) {
			...
			if (config.defaultOpts.customData.storage_type !== 0) {
				elFinder.prototype._options.commands.push('cloudpermissionpublic');
				elFinder.prototype.commands.cloudpermissionpublic = function() {
					this.exec = function(hashes) {
						var self  = this,
							fm    = self.fm,
							files = [];
						$.each(this.files(hashes), function(i, file) {
							files.push(fm.path(file.hash));
						});

						phpR_UCM.fm.account.uploadAcl(fm, files, config.defaultOpts.customData.account_id, config.defaultOpts.customData.bucket, 'public');
					};
				...
				elFinder.prototype._options.commands.push('cloudpermissionprivate');
				elFinder.prototype.commands.cloudpermissionprivate = function() {
					this.exec = function(hashes) {
						var self  = this,
							fm    = self.fm,
							files = [];
						...
						phpR_UCM.fm.account.uploadAcl(fm, files, config.defaultOpts.customData.account_id, config.defaultOpts.customData.bucket, 'private');
					};
					this.getstate = function() {
						return 0;
					}
		...
		load = function() {
			require(
				[
					'elfinder'
					, 'extras/editors.min'               // load text, image editors
					, 'elFinderConfig'
	...
	require.config({
		baseUrl : phprockets_ucm_fm.ucm_fm_assets_url,
		paths : {
