...
if ($type) {
    switch ($type) {
        case 'aws':
            ...
            $aws_url = 'https://' . $aws_config['bucket'] . '.s3.' . $aws_config['region'] . '.amazonaws.com';
            $client = new S3Client([
                    'driver' => 's3',
                    'key' => $aws_config['key'],
        ...
        case 'google_cloud':
            ...
            $storageClient = new StorageClient([
                'projectId' => '502331529493',
                'keyFilePath' => __DIR__. '/My Project-15104a7cf05a.json',
            ]);
            $bucket = $storageClient->bucket('blog-it');
...
//            ];
            $aws_config = [
                'key' => 'CNSFJDPDI25KZVR2KWVB',
                'secret' => 'iHshpbkFH6x0XpIgEbpJz6JUU5eOxmPMejHFeUXOKe8',
                'region' => 'sgp1',
                'bucket' => 'ucm'
            ];
...
//);
$roots = [];
if (isset($cloud_filesystem)) {
    $roots[] = [
        'driver' => 'Flysystem',
        'alias' => $aws_config['bucket'],//Change to anything you like
...
$roots[] = [
    'driver'        => 'LocalFileSystem',           // driver for accessing file system (REQUIRED)
    'path'          => dirname(__DIR__),                 // path to files (REQUIRED)
    'URL'           => dirname($_SERVER['PHP_SELF']) . '/../files/', // URL to files (REQUIRED)
    'trashHash'     => 't1_Lw',                     // elFinder's hash of trash folder
    'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
    'uploadDeny'    => array('all'),                // All Mimetypes not allowed to upload
